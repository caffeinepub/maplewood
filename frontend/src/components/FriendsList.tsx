...
import { X, UserPlus, UserMinus, Search, Users, Loader2 } from 'lucide-react';
...
import { useGetCallerUserProfile, useGetFriendsList, useAddFriend, useRemoveFriend, useSearchProfiles } from '../hooks/useQueries';
      ...
      <div className="panel-dark rounded-lg w-full max-w-md mx-4 border border-neon-orange/30 animate-slide-in">
        <div className="flex items-center justify-between p-4 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-gaming text-neon-orange text-lg tracking-wider">FRIENDS</h2>
          </div>
        ...
        <Tabs defaultValue="friends" className="p-4">
          <TabsList className="grid grid-cols-2 mb-4 bg-muted">
            ...
            <TabsTrigger value="search" className="font-gaming text-xs">FIND PLAYERS</TabsTrigger>
          </TabsList>
            ...
            {isLoading ? (
              ...
              <ScrollArea className="h-64 scrollbar-gaming">
                <div className="space-y-2">
                  {friendIds.map((fid) => (
                    <div key={fid} className="flex items-center justify-between p-2 bg-muted/30 rounded">
                      ...
                      <div key={p.id} className="flex items-center justify-between p-2 bg-muted/30 rounded">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">{p.currentJob}</p>
                          </div>
                        </div>
                        <Button
                          size="sm"
                        ...
                        >
                          {friendIds.includes(p.id) ? 'ADDED' : <><UserPlus className="w-3 h-3 mr-1" /> ADD</>}
                        </Button>
                      </div>
                    ))}
                </div>
