...
export function usePlayerInput(enabled: boolean = true) {
  const inputRef = useRef<PlayerInput>({ ...DEFAULT_INPUT });
  const oneShotRef = useRef<Partial<PlayerInput>>({});

  const getInput = useCallback(() => ({ ...inputRef.current, ...oneShotRef.current }), []);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.code) {
        ...
        case 'KeyE': oneShotRef.current.interact = true; break;
        case 'KeyF': oneShotRef.current.pickup = true; break;
        case 'KeyV': oneShotRef.current.toggleCamera = true; break;
        case 'KeyG': oneShotRef.current.toggleFlight = true; break;
        case 'KeyI': oneShotRef.current.openInventory = true; break;
        case 'KeyM': oneShotRef.current.openMap = true; break;
        case 'Escape': oneShotRef.current.pause = true; break;
        case 'KeyT': oneShotRef.current.chat = true; break;
        case 'Mouse0': case 'KeyQ': oneShotRef.current.attack = true; break;
      }
    };
