...
import { useGetCallerUserProfile, useUpdateJob } from "../hooks/useQueries";
...
const JOBS = [
  ...
  },
  {
    ...
    description:
      "Patrol the streets of Maplewood, respond to crimes, and keep the peace.",
    icon: "👮",
    salary: "$25/hr",
    color: "text-blue-400",
  },
  {
    ...
    title: "SWAT Team",
    description:
      "Elite tactical unit. Respond to high-threat situations and armed standoffs.",
    icon: "🛡️",
    salary: "$40/hr",
    color: "text-red-400",
  },
  {
    ...
    description:
      "Paramedic/firefighter. Respond to medical emergencies and fires across the city.",
    icon: "🚑",
    salary: "$30/hr",
  ...
  },
  {
    ...
    description:
      "Work at Maplewood Auto. Sell cars, conduct test drives, and earn commissions.",
    icon: "🚗",
...
export default function JobBoard({ onClose }: JobBoardProps) {
  ...
  const handleApply = (jobId: GameJob) => {
    ...
    updateJob(jobId, {
      onSuccess: () => {
        toast.success(
          `You are now a ${JOBS.find((j) => j.id === jobId)?.title}!`,
        );
        onClose();
      },
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className={`font-gaming text-sm ${job.color}`}>
                          {job.title}
                        </h3>
                        {isCurrent && (
                      ...
                      <p className="text-xs text-neon-yellow mt-1 font-medium">
                        {job.salary}
                      </p>
                    </div>
                  </div>
