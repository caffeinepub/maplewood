...
actor {
  ...
  public type JobProgress = {
    successfulMissions : Nat;
    failedMissions : Nat;
    roleSpecificStats : RoleSpecificStats;
  };

  public type RoleSpecificStats = {
    #policeOfficer : {
      arrestsMade : Nat;
      finesIssued : Nat;
    };
  ...
  let playerJobProgress = Map.empty<PlayerId, JobProgress>();
  ...
  ///   Role Progress   ///
  /////////////////////////
  // Get caller's job progress
  public query ({ caller }) func getJobProgress() : async JobProgress {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Runtime.trap("Unauthorized: Only users can get job progress");
    };
    let callerId = caller.toText();
    switch (playerJobProgress.get(callerId)) {
      case (null) {
        {
          successfulMissions = 0;
          failedMissions = 0;
          roleSpecificStats = #policeOfficer{
            arrestsMade = 0;
            finesIssued = 0;
          };
        };
  ...
  // Set caller's job progress
  public shared ({ caller }) func setJobProgress(newProgress : JobProgress) : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Runtime.trap("Unauthorized: Only users can update job progress");
    };
    let callerId = caller.toText();
    playerJobProgress.add(callerId, newProgress);
  };

  // Get another user's job progress (caller can view own; admin can view any)
  public query ({ caller }) func getUserJobProgress(user : Principal) : async ?JobProgress {
    if (caller != user and not AccessControl.isAdmin(accessControlState, caller)) {
      Runtime.trap("Unauthorized: Can only view your own job progress");
    };
    let userId = user.toText();
    playerJobProgress.get(userId);
  };

  // Admin can reset a user's job progress
  public shared ({ caller }) func adminResetJobProgress(targetUser : Principal) : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
      Runtime.trap("Unauthorized: Only admins can reset job progress");
    };
    let targetId = targetUser.toText();
    playerJobProgress.remove(targetId);
  };
};
